      ...
      <div class="column-header sticky top-0 px-4 py-6 flex flex-col items-center">
        ...
        ${
            milestone.description
                ...
                : hasAnyDescription
                  ? '<div class="mt-1 text-sm milestone-description-placeholder opacity-0 select-none">&nbsp;</div>'
                  : ""
        }
      </div>
...
export function renderKanban(milestones) {
    ...
    const visibleMilestones = milestones.filter(
        (m) => filterIssues(m.issues).length > 0,
    );

    const html = visibleMilestones
        .map((m, index) => {
    ...
    return (
        html ||
        `<div class="text-center py-10 text-gray-500 dark:text-gray-400">No issues match your filters</div>`
    );
}
