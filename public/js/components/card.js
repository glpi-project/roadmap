...
import { getLuminance, highlightText, hexToRgb, hexToHsl, getPerceivedLightness } from '../utils/helpers.js';
...
function renderLabels(labels) {
  return labels.map(label => {
    ...
    return `<span class="label-pill ${isLight ? 'light-bg' : 'dark-bg'} ${isActive ? 'ring-2 ring-primary-500' : ''}" style="background-color: #${label.color}; ${cssVars}" data-label="${label.name}">${label.name}</span>`;
  }).join('');
}
