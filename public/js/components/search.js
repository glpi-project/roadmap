...
export function populateStatusDropdown() {
  ...
  elements.statusDropdown.innerHTML = options.map((opt, i) => {
...
export function renderActiveFilters() {
  ...
  if (state.filters.projectStatus) {
    const statusOpt = statusOptions.find(o => o.name === state.filters.projectStatus);
  ...
  elements.clearSearch.classList.toggle('hidden', badges.length === 0 && !elements.searchInput.value);
}
...
function showSuggestions(query) {
  ...
  statusOptions.forEach(opt => {
    if (opt.name.toLowerCase().includes(q) && state.filters.projectStatus !== opt.name) {
      const colorKey = opt.color || 'GRAY';
      const colors = STATUS_COLOR_CLASSES[colorKey] || STATUS_COLOR_CLASSES.GRAY;
      suggestions.push({
        type: 'projectStatus',
  ...
  state.allLabels.forEach(label => {
    if (label.name.toLowerCase().includes(q) && !state.filters.labels.includes(label.name)) {
      suggestions.push({
        type: 'label',
        value: label.name,
    ...
    <div class="py-1">
      ${limitedSuggestions.map((s, i) => `
        <button class="suggestion-item w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 ${i === 0 ? 'bg-gray-50 dark:bg-gray-700/50' : ''}" data-type="${s.type}" data-value="${s.value.replace(/"/g, '&quot;')}">
          ...
          <span class="ml-auto text-xs text-gray-400">${s.type === 'projectStatus' ? 'status' : s.type}</span>
        </button>
      `).join('')}
    </div>
  `;
