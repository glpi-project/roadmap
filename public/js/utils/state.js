...
export function sortIssuesByStatus(issues) {
    ...
    return [...issues].sort((a, b) => {
        ...
        const indexA = statusOrder.has(statusA)
            ? statusOrder.get(statusA)
            : statusOptions.length;
        const indexB = statusOrder.has(statusB)
...
export function filterIssues(issues) {
    return issues.filter((issue) => {
        ...
        if (
            state.filters.issueState &&
            issue.state !== state.filters.issueState
        ) {
            return false;
        }
        ...
        if (state.filters.labels.length > 0) {
            ...
            const hasAllLabels = state.filters.labels.every((label) =>
...
export function getTotalIssues() {
    if (!state.roadmapData) return 0;
    return state.roadmapData.milestones.reduce(
